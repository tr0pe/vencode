...
int
getopts(struct arg_s *args, int argc, char *argv[])
{
	char height[8];
	char  width[8];
	char range1[3];
	...
	if(!earg && !darg) {
		fprintf(stderr, "Missing operation option.\nrun '%s -h' for help.\n"
			,argv[0]);
	...
	if(darg) {
		...
		if(!qarg) {
			if(zarg)
				fprintf(stderr, "-z argument is only available with -e "
				        "(file to video). Ignoring.\n");

			if(farg)
				fprintf(stderr,"-f argument is only available with -e "
				        "(file to video). Ignoring.\n");

			if(targ)
				fprintf(stderr,"-t argument is only available with -e "
				        "(file to video). Ignoring.\n");

			if(carg)
				fprintf(stderr,"-c argument is only available with -e "
				        "(file to video). Ignoring.\n");
			if(sarg)
				fprintf(stderr,"-s argument is only available with -e "
				        "(file to video). Ignoring.\n");
		}
		if(sarg) {
			fprintf(stderr,"The FFMPEG process cannot be skipped in decode "
				    "operation (-s argumnt)\n");
			return 1;
		}
		args->codec = NULL;
