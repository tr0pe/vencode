...
FILE *
open_file(const char *filename, unsigned long long *filesize)
{
...
int
compat(const void *a, const void *b)
{
	return strcmp(*(const char **)a,*(const char **)b);
}
int
compat_reverse(const void *a, const void *b)
{
	return strcmp(*(const char **)b,*(const char **)a);
}
void
destroy_filenames(char **filenames,unsigned int count)
{
	count++;
...
void
print_size(unsigned long bytes)
{
	if(bytes >= GB)
		printf("%.2fGb\n",(float)bytes/GB); /* 1Gb */
	else if(bytes >= MB)
...
void
img_index(char *ptr, unsigned int num, pid_t pid, char *ext)
{
